...
while{true}do{
	...
	if (_target isKindOf "Man" && player == vehicle player) then{
		if((side _target == playerSide) && (player distance _target) < __DISTANCE)then{
    ...
    if ((_target isKindOf "Car" || _target isKindOf "Motorcycle" || _target isKindOf "Tank") && player == vehicle player) then{
		if((side _target == playerSide) && (player distance _target) < __DISTANCE && ((count crew _target) > 0))then{
