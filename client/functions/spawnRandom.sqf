...
// @file Author: [404] Deadbeat, [404] Costlyy, [GoT] JoSchaap, [501] Bewilderbeest
...

{
...

    {
		...
		diag_log format["RespawnSelector: Comparing group '%1' with '%2'", _playerGroup, _curPlayerGroup];


		if (_playerSide == "GUER" AND str(_playerGroup) == str(_curPlayerGroup)) then {
			if ((isPlayer _x) AND (str(_curPlayerSide) == _playerSide) AND _x distance _curSpawnPos < 350) then {
	    		//diag_log format["RespawnSelector: fellow indie near %1", _curSpawnLoc select 0];
	    		_spawnAvailable = false;
	    	} else {
	    		//diag_log format["RespawnSelector: no fellow indie near %1", _curSpawnLoc select 0];
	    	};
		};
