...
*/
private ["_name", "_men", "_foundChair", "_fName", "_markerName", "_mPos", "_objects", "_building", "_xName", "_bDeskDirMod", "_pDir", "_base", "_bPos"];
{
		...
		if(damage _building < .5) then
		{
			if(_xName == _fName) then
			{
				...
				switch _fName do
				{
					...
					default{_bPos = (_building buildingPos 0); _bDeskDirMod = 90;};
				};
				_pDir = (getDir _x);
				_base = (getPos _x);
